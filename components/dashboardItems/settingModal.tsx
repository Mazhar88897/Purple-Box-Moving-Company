...
export default function SettingsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-[#004851] border-4 border-[#9A4CB9] rounded-lg p-6 min-w-[300px] relative">
        <button
          onClick={onClose}
          className="absolute top-3 right-3 text-[#9A4CB9] text-2xl font-bold focus:outline-none"
        >
          ×
        </button>
        <h2 className="text-[#9A4CB9] text-xl font-bold mb-6">Setting</h2>
        <div className="flex flex-col gap-4">
          {/* <button className="bg-[#9A4CB9] text-white font-bold py-2 rounded-md">Profile Setting</button> */}
          <button
            className="bg-[#9A4CB9] text-white font-bold py-2 rounded-md"
            onClick={handleResetSettingClick} // Add onClick handler
          >
            Reset Setting
          </button>
          <button onClick={()=>{ sessionStorage.clear();
                router.push('/auth/sign-in');}} className="bg-[#9A4CB9] text-[#004851] font-bold py-2 rounded-md">logout</button>
        </div>
      </div>
